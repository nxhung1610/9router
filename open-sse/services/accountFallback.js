...
export function checkFallbackError(status, errorText, backoffLevel = 0, rotation = null) {
  ...
  //
  // Gate: `rotation.requestShapedNoRotation` (fork setting, default ON) lets a user
  // restore the old fallback behaviour. Callers that pass no `rotation` — notably
  // the combo model ladder — get the classified behaviour unconditionally, which is
  // intended: a payload 400 fails identically on the next MODEL too.
  if (rotation?.requestShapedNoRotation !== false
      && status >= 400 && status < 500 && status !== 401 && status !== 402 && status !== 403 && status !== 429) {
