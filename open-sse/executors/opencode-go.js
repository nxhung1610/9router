...
export class OpenCodeGoExecutor extends DefaultExecutor {
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    // Clamp effort to upstream-supported levels (max/ultra -> xhigh for muse-spark).
    if (out.reasoning && typeof out.reasoning === "object" && !Array.isArray(out.reasoning)) {
      const levels = getThinkingLevels("opencode-go", baseModelId(model || out.model));
      let effort = String(out.reasoning.effort || "").toLowerCase().trim();
      if ((effort === "max" || effort === "ultra") && levels?.length && !levels.includes(effort)) {
        if (effort === "ultra" && levels.includes("max")) out.reasoning.effort = "max";
        else if (levels.includes("xhigh")) out.reasoning.effort = "xhigh";
        else if (levels.length) out.reasoning.effort = levels[0];
      }
    }
    if (out.reasoning && typeof out.reasoning === "object" && !Array.isArray(out.reasoning)) {
